...
int get_tile_el_value(short x,short y)
{
 ...
 else if(sourceFormat==FORMAT_PLANAR4_16x16)	return bytStr[(targetFormat==TARGET_NEOGEO_SPR?tile_x:tile_x/4)*128+(targetFormat==TARGET_NEOGEO_SPR?(x/4)*(1<<((1-full_size)*5)):(tile_x%2)*64)+(targetFormat==TARGET_NEOGEO_SPR?(y/8)*64:((tile_x%4)/2)*32)+(((plan_rev?3-(x%4):x%4)+y*4)<<full_size)];
