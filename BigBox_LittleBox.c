   ...
   if(!match_found)
   {
	...
	for(y=0;y<tile_size;y++)
     ...
     for(x=0;x<tile_size/coef;x++)
	 {
      ...
      if(!(targetFormat==TARGET_OLD_SPRITE||targetFormat==TARGET_NEOGEO_SPR||targetFormat==TARGET_TC0180VCU))	tiles[(isTileMap?unique_tiles:tile_y*tiles_y+tile_x)*(tile_size*tile_size*depth/8)+y*(tile_size*depth/8)+(targetFormat==TARGET_MODEL3_8?(x/4)*4+(3-(x%4)):x)]=pix; //linear target formats
      else
      {
       for(z=0;z<depth;z++)
       {
        if(targetFormat==TARGET_OLD_SPRITE)			tiles[(isTileMap?unique_tiles:tile_y*tiles_y+tile_x)*1024+y*32+(x/4)*4+(7-z)/2]|=((((pix&(1<<z)))>>z)<<(3-(x%4)))<<((1-(z%2))*4);
        else if(targetFormat==TARGET_NEOGEO_SPR)	tiles[tile_x*128+(x/8)*64+y*4+z]|=((pix&(1<<z))>>z)<<(x%8);
        else if(targetFormat==TARGET_TC0180VCU)		tiles[(isTileMap?unique_tile:stile_y*tiles_y+tile_x)*(tile_size*tile_size*depth/8)+y*tile_size+z]|=((pix&(1<<z))>>z)<<(7-(x%8));
	   }
	  }
	 }
    }
